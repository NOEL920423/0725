...
# 讀取銷售資料集（CSV 檔）
yearly_sales_data = pd.read_csv(r"D:\NEAF暑期培訓\lab_2\w2\salesdata.csv")

# your code
# 1. 建立特徵矩陣與標籤向量
X = yearly_sales_data[['DayOfWeek']]  # 特徵：星期幾（1~7）
y = yearly_sales_data['Sales']        # 標籤：對應的銷售金額

# 2. 建立多項式特徵轉換器（例如二次多項式）
poly = PolynomialFeatures(degree=2)   # 你也可以試試 degree=3 或更高
...
# 預測一週（星期一到星期日）的銷售金額
# 計算每星期幾的實際平均銷售金額
X_week = pd.DataFrame(np.arange(1, 8), columns=X.columns)        # 建立 1~7（代表星期一到日）的一維列向量
